def main():
    ...
    if vin in vehicles_dict:
        ...
        user = vehicles_dict[vin]

        # Print the manufacturer, model, and color of the vehicle.
        # Don't print the year, engine design, or displacement.
        manufacturer = user[MANUFACTURER_INDEX]
        model = user[MODEL_INDEX]
        color = user[COLOR_INDEX]
        print(f"Manufacturere: {manufacturer}, Model: {model}, Color: {color}")
